...
REM 2) optional MinIO mirror (same box) + Yandex.Disk offsite copy
...
REM ---- optional offsite copies ----
REM Create C:\Users\minipc\kanban-backup.secret.cmd with any of:
REM   set "S3_ENDPOINT=http://host.docker.internal:9000"   (MinIO, same box)
REM   set "S3_USER=<minio root user>"
REM   set "S3_PASS=<minio root password>"
REM   set "S3_BUCKET=kanban-backups"
REM   set "YANDEX_DISK_TOKEN=<oauth token>"                (Yandex.Disk, true offsite)
...
REM MinIO mirror (second copy on the same machine) — only if S3 creds are present.
if defined S3_USER docker run --rm --add-host host.docker.internal:host-gateway -v "%BACKUP_DIR%":/backups --entrypoint /bin/sh minio/mc -c "mc alias set m %S3_ENDPOINT% %S3_USER% %S3_PASS% >/dev/null 2>&1 && mc mb -p m/%S3_BUCKET% >/dev/null 2>&1; mc mirror --overwrite --remove /backups m/%S3_BUCKET%"

REM Yandex.Disk offsite copy — only if a token is present.
if defined YANDEX_DISK_TOKEN powershell -NoProfile -ExecutionPolicy Bypass -File "%~dp0backup-yandex.ps1" -BackupDir "%BACKUP_DIR%"
:done
endlocal
