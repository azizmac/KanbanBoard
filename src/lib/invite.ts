...
export function makeInviteToken(role: InviteRole = "MEMBER", ttlDays = 7, groupId = ""): string {
  const exp = Date.now() + ttlDays * 86_400_000;
  const payload = Buffer.from(`${role}|${exp}|${groupId}`).toString("base64url");
  return `${payload}.${sign(payload)}`;
}

export function verifyInviteToken(token: string): { role: InviteRole; groupId: string | null } | null {
